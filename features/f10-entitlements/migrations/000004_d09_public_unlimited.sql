-- Forward-only, retry-safe cutover from D07 to D09. NULL means that a
-- commercial quota does not exist; no numeric sentinel represents Unlimited.

ALTER TABLE f10_public_plans
    DROP CONSTRAINT IF EXISTS f10_public_plans_code_check;
ALTER TABLE f10_public_plans
    ALTER COLUMN member_limit DROP NOT NULL,
    ALTER COLUMN channel_limit DROP NOT NULL,
    ALTER COLUMN scheduled_publication_limit DROP NOT NULL;
...
ON CONFLICT (code) DO UPDATE
    SET display_name = EXCLUDED.display_name,
        ...
        scheduled_publication_limit =
            EXCLUDED.scheduled_publication_limit;

ALTER TABLE f10_public_plans
    DROP CONSTRAINT IF EXISTS f10_public_plans_d09_limits_check,
    DROP CONSTRAINT IF EXISTS f10_public_plans_d09_annual_check;
ALTER TABLE f10_public_plans
    ADD CONSTRAINT f10_public_plans_code_check
    CHECK (code IN ('start', 'pro', 'team', 'unlimited')),
    ADD CONSTRAINT f10_public_plans_d09_limits_check
    CHECK (
        (
            code = 'unlimited'
        ...
        OR (
            ...
            AND member_limit > 0
            AND channel_limit > 0
            AND scheduled_publication_limit > 0
        )
    ),
    ADD CONSTRAINT f10_public_plans_d09_annual_check
    CHECK (annual_price_cents = monthly_price_cents * 10);

ALTER TABLE f10_workspace_billing
    ALTER COLUMN channel_quantity DROP NOT NULL,
...
UPDATE f10_workspace_billing
   SET channel_quantity = CASE
       WHEN billing_state = 'trialing' THEN least(channel_quantity, 9)
       WHEN plan_code = 'start' THEN least(channel_quantity, 3)
       WHEN plan_code = 'pro' THEN least(channel_quantity, 6)
       WHEN plan_code = 'team' THEN least(channel_quantity, 9)
       WHEN plan_code = 'unlimited' THEN NULL
   END
 WHERE plan_code IN ('start', 'pro', 'team', 'unlimited');

ALTER TABLE f10_workspace_billing
    ADD CONSTRAINT f10_workspace_billing_channel_quantity_check
    CHECK (
        (plan_code = 'unlimited' AND channel_quantity IS NULL)
        OR (
            billing_state = 'trialing'
            AND channel_quantity BETWEEN 1 AND 9
        )
        OR (
            billing_state <> 'trialing'
            AND (
                (plan_code = 'start' AND channel_quantity BETWEEN 1 AND 3)
                OR (plan_code = 'pro' AND channel_quantity BETWEEN 1 AND 6)
                OR (plan_code = 'team' AND channel_quantity BETWEEN 1 AND 9)
            )
        )
    );
...
ALTER TABLE f10_checkout_sessions
    ...
    CHECK (
        (plan_code = 'unlimited' AND channel_quantity IS NULL)
        OR (plan_code = 'start' AND channel_quantity BETWEEN 1 AND 3)
        -- Historical D07 checkout rows are immutable evidence and may retain
        -- quantities above D09. The registration function below rejects such
        -- quantities for every new checkout.
        OR (plan_code IN ('pro', 'team') AND channel_quantity BETWEEN 1 AND 50)
    );

ALTER TABLE f10_paddle_catalog_audit
    DROP CONSTRAINT IF EXISTS f10_paddle_catalog_audit_plan_code_check,
    DROP CONSTRAINT IF EXISTS f10_paddle_catalog_audit_tier_check;
ALTER TABLE f10_paddle_catalog_audit
    ADD CONSTRAINT f10_paddle_catalog_audit_plan_code_check
    CHECK (plan_code IN ('pro', 'team', 'unlimited')),
    ADD CONSTRAINT f10_paddle_catalog_audit_tier_check
    CHECK (
        (plan_code IN ('pro', 'team') AND tier IN ('1-10', '11-25', '26-50'))
        OR (plan_code = 'unlimited' AND tier = 'flat')
    );
...
BEGIN
    ...
    IF v_billing.billing_state = 'trialing' THEN
        v_limit := CASE p_resource
            WHEN 'members' THEN 9
            WHEN 'channels' THEN 9
            WHEN 'scheduled_publications' THEN 500
        END;
    ...
    SELECT billing.*,
           ...
           CASE resources.resource
               WHEN 'members' THEN CASE
                   WHEN billing.billing_state = 'trialing' THEN 9
               ...
               END
               WHEN 'scheduled_publications' THEN CASE
                   WHEN billing.billing_state = 'trialing' THEN 500
                   ELSE plans.scheduled_publication_limit
               END
           END AS quota_limit
