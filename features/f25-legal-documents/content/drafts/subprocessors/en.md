...
| Apple Inc. | Independent third party | OAuth login ("Sign in with Apple") | Email address (or Apple private relay email); name (first sign-in only); Apple account identifier | United States | Not verified | Not verified | Not applicable — no dedicated DPA published for this feature | [Sign in with Apple & Privacy](https://www.apple.com/legal/privacy/data/en/sign-in-with-apple/) |
| Meta Platforms, Inc.; Meta Platforms Ireland Limited | Independent third party | OAuth login ("Facebook Login") and the customer's own connection to Facebook Pages / Instagram Professional as a publishing destination | Email address; name; profile picture; Facebook/Instagram account identifier; content the customer chooses to publish to their connected account | Ireland; United States | Not verified | Standard Contractual Clauses; Meta Platforms, Inc. also Data Privacy Framework certified | Not applicable — no dedicated DPA published for this feature | [Meta Platform Terms](https://developers.facebook.com/terms/dfc_platform_terms/) |
...
- **Mailronix / Apdsoftware di Carlo Zuffetti**: the official page mailronix.com/terms publicly states the operating entity as "Apdsoftware di Carlo Zuffetti", Via C. Colombo 15, 24047 Treviglio (BG), Italia, P.IVA 03835250162, and states that the transaction descriptor "può essere comune ad altri prodotti della stessa ditta" (may be shared with other products of the same firm) — consistent with "APDSoftware" being the same brand referenced elsewhere in this corpus as Postqron's operator, though mailronix.com's own page does not name Postqron specifically. **Item to resolve during legal review**: if Mailronix and Postqron are confirmed to be operated by the identical legal entity, this is not an ordinary arm's-length subprocessor relationship between independent parties — it is the same controller/processor using a separately branded system, which changes how Art. 28 GDPR applies (an entity cannot be its own sub-processor) and requires precise characterization by counsel rather than the ordinary vendor classification retained here provisionally.
- **Apple and Meta processing-location statements** were not found on the official pages reviewed and require direct confirmation before publication.
