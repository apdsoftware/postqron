...
- **Social channels** (`/app/social-channels`) renders the complete D2 provider
  ...
  Mastodon and Bluesky remain visible but disabled until #324 provides the
  authoritative dynamic-discovery/PAR/DPoP contract needed by #314. F30 does
  not invent instance, handle, DID, PDS, nonce, or app-password inputs.
- **Publish** (`/app/publish`) consumes only F6 0.2.0 (#303 / PR #317) for the
  ...
  scheduling contract can enqueue it safely. When editing, the same scheduled
  post is always rescheduled and a second post is never created.
