...
test('landing includes SEO, legal, support and accessible CTA contracts', async () => {
  ...
  assert.match(pricing, /pricing-model\.ts/u)
  assert.match(pricing, /initialPricingSelection/u)
  assert.match(pricing, /orderedPlans/u)
  assert.match(pricing, /role="radiogroup"/u)
  assert.match(pricing, /props\.accessUrl/u)
  assert.match(pricing, /pricing\.note/u)
  assert.doesNotMatch(pricing, /checkoutIntentFor|purchaseHref/u)
