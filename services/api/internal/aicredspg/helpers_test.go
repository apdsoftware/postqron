...
// messaggio di skip dice cosa non è stato verificato, perché un salto silenzioso
// darebbe l'impressione che le query siano state provate quando non lo sono
// state.
func newTestDatabase(t *testing.T) *pgxpool.Pool {
	...
	pool, err := database.Open(ctx, target, database.Options{
		MaxConns:        4,
		ApplicationName: "postqron-test-aicredspg",
	})
	if err != nil {
...
func testDatabaseName(t *testing.T) string {
	...
	name := fmt.Sprintf("pq_aicredspg_%d_%d_%s", os.Getpid(), databaseCounter.Add(1), sanitized)
	if len(name) > 63 {
		name = name[:63]
	}
	return name
...
// AI hanno una chiave esterna su `users`: senza un utente vero non si inserisce
// niente.
