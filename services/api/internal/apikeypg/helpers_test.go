...
func newTestDatabase(t *testing.T) *pgxpool.Pool {
	...
	pool, err := database.Open(ctx, target, database.Options{
		MaxConns:        4,
		ApplicationName: "postqron-test-apikeypg",
	})
	if err != nil {
...
func testDatabaseName(t *testing.T) string {
	...
	name := fmt.Sprintf("pq_apikeypg_%d_%d_%s", os.Getpid(), databaseCounter.Add(1), sanitized)
	if len(name) > 63 {
		name = name[:63]
	}
	return name
...
// createUser inserisce un account e ne restituisce l'identificativo. Le chiavi
// hanno una chiave esterna su `users`: senza un utente vero non si inserisce
// niente.
func createUser(t *testing.T, pool *pgxpool.Pool, email string) string {
	t.Helper()
	var id string
	err := pool.QueryRow(t.Context(),
		`INSERT INTO users (email, password_hash) VALUES ($1, 'hash-di-prova') RETURNING id::text`,
		email).Scan(&id)
	if err != nil {
		t.Fatalf("creazione dell'account di prova: %v", err)
	}
	return id
}
