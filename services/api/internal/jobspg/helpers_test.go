...
// `make ci` deve restare verde su una macchina senza `make db-up`. Il messaggio di
// skip dice cosa non è stato verificato, perché un salto silenzioso darebbe
// l'impressione che le query siano state provate quando non lo sono state.
//
// È qui che si verificano le proprietà che internal/jobs non può provare da
// solo: il vincolo XOR fra `schedule` ed `every_seconds`, l'unicità del nome,
// l'atomicità del tetto di piano dentro l'INSERT e la chiave naturale di
// `job_executions` che fa da lock al trigger manuale.
func newTestDatabase(t *testing.T) *pgxpool.Pool {
	...
	pool, err := database.Open(ctx, target, database.Options{
		MaxConns:        4,
		ApplicationName: "postqron-test-jobspg",
	})
	if err != nil {
...
func testDatabaseName(t *testing.T) string {
	...
	name := fmt.Sprintf("pq_jobspg_%d_%d_%s", os.Getpid(), databaseCounter.Add(1), sanitized)
	if len(name) > 63 {
		name = name[:63]
	}
	return name
...
func applyMigrations(t *testing.T, pool *pgxpool.Pool) *migrate.Migrator {
	t.Helper()
