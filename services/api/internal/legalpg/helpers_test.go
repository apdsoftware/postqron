...
// prova del consenso non si possa riscrivere, e un salto silenzioso darebbe
// l'impressione che quella verifica sia stata fatta.
func newTestDatabase(t *testing.T) *pgxpool.Pool {
	...
	pool, err := database.Open(ctx, target, database.Options{
		MaxConns:        4,
		ApplicationName: "postqron-test-legalpg",
	})
	if err != nil {
...
func testDatabaseName(t *testing.T) string {
	...
	name := fmt.Sprintf("pq_legalpg_%d_%d_%s", os.Getpid(), databaseCounter.Add(1), sanitized)
	if len(name) > 63 {
		name = name[:63]
	}
	return name
...
func newStore(t *testing.T) (*legalpg.Store, *pgxpool.Pool) {
	t.Helper()
	pool := newTestDatabase(t)
	store, err := legalpg.New(pool)
	if err != nil {
...
// nuovoUtente crea un account minimo e ne restituisce l'identificativo.
func nuovoUtente(t *testing.T, pool *pgxpool.Pool, slug string) string {
	t.Helper()
	var id string
	err := pool.QueryRow(t.Context(),
		`INSERT INTO users (email, password_hash, full_name)
		 VALUES ($1, 'argon2id$prova', $2) RETURNING id::text`,
		slug+"@esempio-consenso.test", "Nome "+slug).Scan(&id)
	if err != nil {
		t.Fatalf("creazione dell'utente di prova: %v", err)
	}
	return id
}

func conta(t *testing.T, pool *pgxpool.Pool, sql string, args ...any) int {
	t.Helper()
	var n int
