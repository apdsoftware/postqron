...
// state — e qui la cosa non provata sarebbe la traccia del consenso, cioè la
// prova del diritto di scrivere che la privacy policy §2.8 promette di tenere.
func newTestDatabase(t *testing.T) *pgxpool.Pool {
	...
	pool, err := database.Open(ctx, target, database.Options{
		MaxConns:        4,
		ApplicationName: "postqron-test-marketingpg",
	})
	if err != nil {
...
func testDatabaseName(t *testing.T) string {
	...
	name := fmt.Sprintf("pq_mktpg_%d_%d_%s", os.Getpid(), databaseCounter.Add(1), sanitized)
	if len(name) > 63 {
		name = name[:63]
	}
	return name
