...
// state — e qui la cosa non provata sarebbe l'idempotenza di R16.
func newTestDatabase(t *testing.T) *pgxpool.Pool {
	...
	pool, err := database.Open(ctx, target, database.Options{
		MaxConns:        4,
		ApplicationName: "postqron-test-paddlepg",
	})
	if err != nil {
...
func testDatabaseName(t *testing.T) string {
	...
	name := fmt.Sprintf("pq_paddlepg_%d_%d_%s", os.Getpid(), databaseCounter.Add(1), sanitized)
	if len(name) > 63 {
		name = name[:63]
	}
	return name
